...
CREATE OR ALTER PROCEDURE [Definition].[AddMemberToRoleByName]
      @db_name      SYSNAME
    , @role_name    SYSNAME
    , @username     SYSNAME
    , @debug        BIT = 1
...
BEGIN TRY
    ...
    SELECT
        @sql =
            ...
            ADD MEMBER ' + QUOTENAME(@username, '"')
            + @sql_end
    IF @debug = 1 PRINT '-- Adding user: ' + QUOTENAME(@username, '"') + 'to role: ' + QUOTENAME(@role_name)
