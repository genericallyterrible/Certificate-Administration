...
-- Description:     Creates a certificate from a SourceObjectCertificate,
--                  creates a user from the new certificate,
--                  and appends the certificate's ID to the 
--                  TargetObjectCertificate table.
-- Requires:        SA
-- ======================================================================

CREATE OR ALTER PROCEDURE [Permission].[CreateTargetObjectCertificateAndUser]
...
BEGIN TRY
    ...
    SELECT
        ...
        , @src_cert_id = MIN([soc].[certificate_id])
    FROM
    ...
    WHERE [soc].[SourceObjectCertificateID] = @SourceObjectCertificateID

    -- RAISERROR if a certificate is not found
    IF @src_cert_id IS NULL
        RAISERROR('No SourceObjectCertificate exists with ID: "%d"', 16, 1, @SourceObjectCertificateID)
    ...
    -- Log the certificate
    INSERT INTO [TargetObjectCertificate] (
