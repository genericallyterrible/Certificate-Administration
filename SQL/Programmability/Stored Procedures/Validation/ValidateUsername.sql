...
CREATE OR ALTER PROCEDURE [Validation].[ValidateUsername]
      @db_name      SYSNAME
...
BEGIN TRY
    ...
    SELECT
        ...
        , @params = 
            '@username SYSNAME OUTPUT'
    IF @debug = 1 PRINT '-- Validating user :"' + @username + '"'
    ...
    BEGIN
        IF @raiserror = 1
            RAISERROR('No user: "%s"', 16, 1, @origname)
        IF @debug = 1
            PRINT '-- No user: ' + QUOTENAME(@origname, '"')
                + CHAR(13) + CHAR(10)
    END
